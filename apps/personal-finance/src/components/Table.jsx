...
export default function Table({
  ...
  const renderCell = (col, content) => {
    ...
    return (
      <div
        className={`truncate ${alignment} overflow-hidden text-ellipsis whitespace-nowrap`}
  ...
  // Generate gridTemplateColumns style
  const columnWidths = columns.map((col) => col.width || '1fr').join(' ')
  const gridStyle = { gridTemplateColumns: columnWidths }

  return (
    <div
      className={`flex flex-col h-full overflow-hidden ${borderRadius} shadow bg-[var(--color-card)]`}
    >
      {!hideHeader && (
        <div
          className="px-[16px] py-[12px] border-b border-[var(--color-muted)] bg-[var(--color-inner-card)] text-[12px] font-bold capitalize text-[var(--color-text-hover)] grid"
          style={gridStyle}
        >
          {columns.map((col) => (
            <div key={col.key}>{renderCell(col, col.label)}</div>
          ))}
        </div>
      )}
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto">
        {rows.length === 0 ? (
          <div className="text-center text-[13px] text-[var(--color-text-hover)] mt-[40px]">
          ...
          rows.map((row, idx) => (
            <div
              key={idx}
              className={`grid h-[45px] px-[16px] py-[12px] text-[11px] font-semibold text-[var(--color-text)] border-b border-[var(--color-muted)] hover:bg-[var(--color-muted-hover)] transition`}
              style={gridStyle}
            >
              {columns.map((col) => {
                if (col.key === 'date') {
                  return (
                    <div key={col.key}>{renderCell(col, formatDate(row[col.key]))}</div>
                  )
                }
                if (col.key === 'amount') {
                ...
                if (col.key === 'category') {
                  const category =
                    row.category_override || row.category || 'Uncategorized'
                  ...
                  const dropdownOptions = Object.entries(categoryStyles).map(
                      ...
                      <DropdownMenu
                        ...
                        trigger={
                          ...
                          >
                            {category}
                            <span
                              className={`absolute right-[6px] top-[12px] -translate-y-1/2 text-[var(--color-text)] transition-opacity duration-150 ${
                                isEditing
                                  ? 'opacity-100'
                        ...
                        onSelect={async (selected) => {
                          const overrideValue =
                            selected === row.category ? null : selected
                ...
                return <div key={col.key}>{renderCell(col, row[col.key])}</div>
              })}
            </div>
          ))
        )}
      ...
      {!hideFooter && (
        <div className="flex select-none justify-between items-center gap-[12px] px-[16px] py-[12px] border-t border-[var(--color-muted)] text-[12px] text-[var(--color-text-hover)]">
          <div className="flex items-center gap-[8px] text-[11px] font-medium">
            ...
            />
            <span>
              Viewing {(currentPage - 1) * 20 + 1} –{' '}
              {Math.min(currentPage * 20, totalPages * 20)} of{' '}
              {totalPages * 20}
            </span>
          </div>
